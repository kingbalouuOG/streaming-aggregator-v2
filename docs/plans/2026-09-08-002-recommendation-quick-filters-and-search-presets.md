...
- Logging — call `emitSearch('', itemsVisible, { mode: 'filter', metadata: { surface, category, rails_visible, items_visible } })` from the store on every change (Session 1 will have wired `emitSearch` on native).
