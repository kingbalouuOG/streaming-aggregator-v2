...
**Status:** v0.7 — Phase Search V2 close-out. §3.4 captures Phase Search V2's single migration (041 ✅ Applied 2026-05-13). No process changes from v0.6. Phase Search V2 summary at `docs/v2/phase-summaries/phase-search-v2-summary.md` is authoritative. Note: Phase Search V2 ran **before** Phase 5.5 (reordered from the kickoff brief's "5.5 first" assumption); 5.5 picks up next.

> **Cross-phase knowledge base.** Active strategy / orchestration / parking-lot / phase-summary documents live under `docs/v2/` in this repo (source of truth). A separate **LLM-maintained Karpathy-style wiki** sits at `C:\Users\User\Documents\Code\StreamingAggregatorV2\videx-wiki` (sibling to the repo, not version-controlled with it) — it synthesises the `docs/v2/` material plus runbooks / post-mortems / source-doc snapshots into a queryable knowledge graph (entities, concepts, registers). **Read `videx-wiki/AGENTS.md` first** before operating in that vault — it defines the schema and the three workflows (ingest / query / lint). The wiki's last ingest was Phase 5 close-out (2026-05-07); Phase 5.5 ingest pending.
**Version:** 0.7
...
| 040 | `040_available_tmdb_ids_with_media_type.sql` | Phase 5 (IN-458) | ⏳ Deferred to Phase 5.5 / 6 | Extend `get_available_tmdb_ids` to return `(tmdb_id, media_type)` pairs. Closes 0.8% movie/TV id collision rate measured in Phase 4 audit. Real but small impact; deferred to keep Phase 5 scope tight. |
