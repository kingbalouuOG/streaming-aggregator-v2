...
**Date:** April 2026
**Status:** v1.8 — IN-466 lock: For You first paint runs server-side via the `render-foryou-rows` Edge Function (ADR-012). One client → Edge call replaces 5-8 sequential client → Postgres round trips. Existing client pipeline retained as fallback path.
**Version:** 1.8

**Changes from v1.7:**
- §5.4 added: Server-side render layer for For You first paint. Documents the Edge Function commitment, the auth model (service-role + manual JWT decode + `withUserScope` helper after the auth-spike showed RLS overhead at ~280ms in the critical path), the fallback contract (1.5s timeout → existing client pipeline), Variant A warm-pinger, and the ADR-011-compliant `_shared/` mirror with CI-enforced drift control.
- §9.5 added: For You server-side render commits (ADR-012 lock + Variant A warm-pinger + IN-467/468/469 follow-ups filed in Parking Lot v0.5).
- IN-466 status flipped from ⏳ Quick wins shipped → ✅ Incorporated in Parking Lot v0.5; three follow-ups (IN-467 mirror consolidation, IN-468 Variant B SWR cache, IN-469 cold-start mitigation continuation) filed.

**Changes from v1.6.3:**
- §5.2 (Mood Rooms): added a subsection on the title-anchored ranking layer that ships on the For You "Mood Rooms for Tonight" row. Global HDBSCAN clustering continues monthly; global rooms persist in `mood_rooms`/`mood_room_titles` for v2.5 browse and Phase 7 conversational discovery.
- §9.3 (Mood Rooms commits): item 17 amended to reflect anchored ranking on For You. Item 16 (HDBSCAN clustering pipeline) unchanged. New item 17a covers the reusable room-generation primitive at `src/lib/recommendations-v2/anchoredRoom.ts` and its planned re-use by the Phase 6+ detail-page "Make a room from this title" affordance (IN-464).
- §5.2.1: instrumentation note for migration 033 — `card_impressions.metadata` jsonb column + `'anchor_room'` source_surface — used by anchored mood rooms (`anchor_tier`, `anchor_source_cluster_id`, `tier_1_inside_stated_cluster`). Migration registered in Project Orchestration v0.4 §3.4.
- §8.2: new open question on whether the cluster-coherence guard's 0.40 cosine-distance threshold and 5-interaction cold-start window land correctly under real usage. Tunable in `src/lib/recommendations-v2/anchorSelection.ts`; surface examples mid-flight if early data suggests recalibration.
...
**Phase 4 implementation (shipped):** The 50/20/10/10/10 table represents intent. In implementation, the three *scoring* components (taste, recency, contextual) form the Stage 2 weighted sum normalised to 1.0 — taste 62.5% / recency 25% / contextual 12.5% — while intra-list diversity and cross-service spread are implemented as **post-processing stages** (genre-spread + de-clustering), not as literal scoring components. Weight constants live in `src/lib/recommendations-v2/weights.ts`. The Catalogue-age slider modulates recency weight between 10-30% with proportional re-normalisation of the other two components.

**Phase 5 update (shipped 2026-05-06):** the contextual component is no longer a placeholder — `contextual.ts` now produces a real 0–1 score per candidate from time-of-day, viewing-context, and device sub-scorers (40% / 40% / 20% within the contextual budget). Intra-row diversity moved from `applyGenreSpread` to greedy MMR over 1536D embeddings (`applyGenreSpread` retained as the embeddings-absent fallback). `BASE_WEIGHTS` (62.5/25/12.5) is unchanged in Phase 5; weight-split re-evaluation is deferred until prototype-user vectors rebase through the next 24h taste-recompute cycle (a latent bug in `INTERACTION_WEIGHTS` was also fixed in Phase 5 — see phase summary). Strategy v1.9 will land with the chosen split. Reference: `docs/v2/phase-summaries/phase-5-summary.md`.
...
**Vector store vs row selection interaction:** the For You "Mood Rooms for Tonight" row no longer queries `mood_rooms` and `mood_room_titles` directly — it now generates anchored rooms on demand around 5 user-specific anchor titles per week. Global rooms continue to be produced by the monthly clustering pipeline and are reserved for v2.5 browse and Phase 7 conversational discovery (see §5.2.1 below).

#### 5.2.1 Title-anchored mood rooms ranking layer (Phase 4.5 redirect)

Locked April 2026 after Phase 4.5 Gates 1–4 surfaced a centroid-flattening problem in the global cosine-distance ranking — niche cluster picks (history-war, true-crime, epic-scifi, mind-bending) were being ranked below tonally-adjacent generic rooms (Saturday Night Action, Slow-Burn Horror) because the user's averaged taste vector landed nearest the densest neighbourhoods. Diagnostic: `docs/v2/Mood_Rooms_Ranking_Quality_Investigation_2026_04_22.md`. Probe: `docs/v2/Mood_Rooms_Anchored_Probe_2026_04_26.md`.

The For You "Mood Rooms for Tonight" row now generates rooms around 5 user-specific anchor titles per weekly refresh. Each room is a 30-title neighbourhood around its anchor in 1536D embedding space, named "If you love {anchor}" in v1 (LLM labelling deferred to IN-463).

**Anchor selection ladder (`src/lib/recommendations-v2/anchorSelection.ts`):**

- **Tier 1 — Behavioural-positive intersection.** `thumbs_up ∩ (watched ∪ watchlist_add)` over the last 60 days. Three guards: (G1) combined-signal requirement; (G2) similarity gate `cos_sim(taste, anchor) >= 0.55`; (G3) cluster-coherence gate, active only when `taste_vector_interaction_count <= 5`, requires the anchor's embedding to sit within `cosine_distance <= 0.40` of at least one user-selected cluster centroid. The cold-start guard prevents a single off-pattern thumbs-up from dominating a thin behavioural row; once the user has accumulated meaningful behavioural signal, the guard bypasses and behaviour wins over stated picks.
- **Tier 2 — Cluster representative titles ranked by similarity to the user vector.** One per cluster, top-similarity rep wins.
- **Tier 3 — Top-finalScore fallback** from the existing `fetchCandidatePool` + `scoreCandidates` pipeline, gated by `finalScore > 0.65`. Only fires if Tier 1+2 yield < 5 anchors.

A cross-tier collision rule prevents a Tier 1 horror anchor from coexisting with a Tier 2 horror cluster anchor in the same row: each Tier 1 anchor "occupies" any cluster whose centroid sits within 0.40 cosine-distance, and Tier 2 picks skip occupied clusters.

**Reusable primitive.** Per-anchor room generation is exported from `src/lib/recommendations-v2/anchoredRoom.ts` as `buildAnchoredRoom(anchorTmdbId, anchorMediaType, filterSets, options)`. The For You row consumes it; the Phase 6+ detail-page "Make a room from this title" affordance (IN-464) will consume the same primitive. The Because You Watched row was migrated to this primitive in the same Phase 4.5 redirect.

**Persistence and caching.** No `user_anchor_rooms` table — rooms are derived on demand. Anchor selections are cached in localStorage by `(userId, weekBucket)` mirroring the global rooms hook's pattern, with `featuredLastWeek` exclusion. Room contents are recomputed every render from the cached anchor IDs.

**Instrumentation.** Migration 033 adds `card_impressions.metadata` (jsonb) and extends `source_surface` with `'anchor_room'`. Per-impression metadata captures `anchor_tmdb_id`, `anchor_tier` (1/2/3), `anchor_source_cluster_id` (Tier 2 only), and `tier_1_inside_stated_cluster` (Tier 1 only). This data answers IN-OB-006 (onboarding cluster taxonomy review at Phase 6) with telemetry instead of guesswork: frequency of Tier 1 anchors falling outside any user-selected cluster, per-cluster anchor surfacing rate, CTR by anchor source tier.

**What's deferred.** LLM-generated thematic room labels (IN-463, Phase 4.5 fast-follow); detail-page "Make a room from this title" (IN-464, Phase 6+); per-user room persistence (deferred indefinitely — synthesise-on-demand is sufficient); onboarding cluster taxonomy redesign (IN-OB-006, Phase 6 review pending Phase 4.5 telemetry).

**Global rooms infrastructure unchanged.** The monthly HDBSCAN reclustering pipeline continues to run untouched. Global rooms persist in `mood_rooms`/`mood_room_titles` and remain available for the v2.5 dedicated browse surface and Phase 7 conversational discovery. Schema-level posture: no data deletion, no clustering pipeline changes.

### 5.4 Server-side render layer for For You first paint (IN-466, ADR-012)

The For You surface's first-paint data path is collapsed into a single Edge Function call. Pre-IN-466, `useForYouContent` fired 5-8 sequential client → Postgres round trips (taste profile + filter sets ×4 + candidate pool RPC + extended metadata + anchor selection ×5 + per-anchor room generation ×5). The cold path was ~2-3s on residential WAN even after Phase 4.5 quick wins (migration 035 JSONB-array RPC, localStorage availability cache, label cache).

`render-foryou-rows` Edge Function returns the full first-viewport payload — Recommended For You, Hidden Gems, Outside Your Usual, Because You Watched, More From [Person], From Your Watchlist, 5 anchored mood rooms with thumbnails + cached LLM labels, plus the 500-candidate pool for client-side slider rerank — in one JSON response. Warm wallclock lands at ~850ms p50 / ~1300ms p95.

**Auth model: service-role + manual JWT decode + `withUserScope(uid)` helper.** The auth-spike (`supabase/functions/auth-spike/`) measured user-JWT-scoped reads at ~80ms warm per query plus ~80ms `getUser` overhead. Across the render function's 5-8 critical-path queries, that's ~280ms of pure auth/RLS cost in a 600ms p50 budget — too much. Service-role bypasses RLS, so every user-owned read goes through `userScope.select(table, cols)` which auto-applies `.eq('user_id', uid)`. The wrapper is the defence-in-depth replacement for RLS. Tables without a `user_id` column (titles, mood_room_anchor_labels, streaming_availability) use the raw client.

**Fallback contract: 1.5s timeout → client pipeline.** Cold Edge Function instances take 5-12s to start. The brief's 5s timeout was tightened to 1.5s after the cold profile came in worse than expected: a 5s timeout means the user waits 5s + 2-3s client fallback = 7-8s in the cold case, vs 2-3s for client-direct. 1.5s lets warm requests succeed (~800ms typical) while bailing fast on cold.

**Variant A warm-pinger (`warmup-foryou`).** Fired from `App.tsx` at mount. Wakes the Deno runtime + DB connection + pgvector HNSW index so the subsequent For You navigation hits a warm path. Closes the cold-start gap for users who don't tap For You within ~2 seconds of app open. Variant B (localStorage SWR snapshot) and pg_cron-driven warm-pinger filed as IN-468 / IN-469 — deferred pending production cold-start telemetry.

**Pipeline code mirroring (ADR-011 + IN-467 follow-up).** `supabase/functions/_shared/recommendations-v2/` and `_shared/taste-v2/` mirror `src/lib/recommendations-v2/` and `src/lib/taste-v2/` exactly. The `shared-tree-drift` GitHub Actions workflow fails any PR that touches one tree without the other. Escape hatch: `drift-allowed: <reason>` marker in the PR body for emergency one-sided patches. Long-term consolidation into a runtime-portable shared package is filed as IN-467, revisit after 1-2 months of operational experience.

### 5.5 What we explicitly do NOT build in v2

(Renumbered from §5.3 to make room for §5.4.)
...
**Phase 5 — Contextual signals + pre-launch hardening.** ✅ Complete (2026-05-06). Real contextual scorer (time-of-day 40% / viewing-context 40% / device 20% within the contextual budget). MMR upgrade replaces `applyGenreSpread` as primary intra-row diversity (genre-spread retained as fallback). Pre-launch security blockers: taste_profiles RLS, marked_watched cleanup, username_available SECURITY DEFINER RPC, CORS tightening, verify_jwt CI guard, cron jobs Vault-backed JWT (storage hygiene; cryptographic rotation deferred to Phase 6 pending Supabase tooling). `<Database>` generic re-enabled. Five migrations (036–039 applied; 040 deferred). Weight-split re-evaluation is the one outstanding Phase 5 deliverable, deferred until prototype vectors rebase post-marked_watched fix; will be tuned and locked in Strategy v1.9. See `docs/v2/phase-summaries/phase-5-summary.md`.
...
10. **Anchored mood rooms guard thresholds — are they calibrated correctly?** The cluster-coherence guard's `cosine_distance <= 0.40` and the cold-start window's `interaction_count <= 5` are first-pass values from the probe. Surface examples mid-flight if Tier 1 anchors are getting demoted that look legitimate, or if off-pattern anchors are slipping through. Tunable in `src/lib/recommendations-v2/anchorSelection.ts` (constants at top of file).
...
16. **Mood Rooms as Pillar 1 USP** — HDBSCAN clustering via Python + GitHub Actions monthly cron, two-pass LLM labelling, monthly re-clustering with stability constraints. Pipeline unchanged after the Phase 4.5 redirect.
17. **For You "Mood Rooms for Tonight" row uses title-anchored ranking** (locked April 2026, Phase 4.5 redirect). Five anchors per user per week from a tiered ladder: Tier 1 behavioural intersection with three guards, Tier 2 cluster representatives, Tier 3 top-finalScore fallback. Rooms named "If you love {anchor}" in v1; LLM labelling deferred to IN-463. Global rooms persist for v2.5 browse and Phase 7 conversational discovery — they no longer back the For You row but the underlying infrastructure (tables, RPCs, monthly clustering job) is untouched.
17a. **Reusable room-generation primitive** at `src/lib/recommendations-v2/anchoredRoom.ts`. Consumed by both the For You anchored row and the Because You Watched row in Phase 4.5. Phase 6+ detail-page "Make a room from this title" (IN-464) will consume the same primitive.
...
38. **For You first paint runs server-side** via the `render-foryou-rows` Edge Function (ADR-012, IN-466 lock April 2026). One client → Edge call replaces 5-8 sequential client → Postgres round trips. Existing `useForYouContent` client pipeline retained as the fallback path with 1.5s timeout. Auth: service-role + manual JWT decode + `withUserScope` helper. Variant A warm-pinger (`warmup-foryou`) fires from `App.tsx` mount.
39. **Pipeline code mirrored into `supabase/functions/_shared/`** per ADR-011, with `shared-tree-drift` GitHub Actions workflow enforcing lockstep changes. Long-term consolidation filed as IN-467.

---

*End of strategy document v1.8. IN-466 server-side render lock incorporated. All architectural decisions from CC review rounds 1-3 plus the Phase 4.5 redirect plus IN-466 are locked. Ready for next phase planning.*
