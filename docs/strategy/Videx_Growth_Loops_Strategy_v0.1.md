...
| Area | State today |
|---|---|
| Share, invite, referral | None in the codebase |
| Routing | No router; titles, mood rooms and watchlists have no URL |
| Link previews | One static Open Graph tag set ("Your personal streaming guide") for the whole site |
| App links / universal links | None |
| Auth | Email and password only |
| Web presence | Cloudflare Worker (`videx-api`) exists; no public content pages |
| Retention loop | Push notifications v1: watchlist arrivals and leaving-soon, one bundled push per user per day, Expo push via the native (Expo) app |
...
Summary: Videx has strong loop raw material and zero loop plumbing.
...
1. Domain and web presence: is there a public domain and a marketing page today, or does G0 include standing one up?
2. iOS status: is the Expo app shipping on iOS as well as Android? Universal links and Apple sign-in are moot until it is.
