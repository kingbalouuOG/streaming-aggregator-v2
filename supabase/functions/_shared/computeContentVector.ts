/**
 * Compute Content Vector (Shared Module)
 *
 * Pure function that maps TMDb content metadata to a TasteVector.
 * No browser APIs, no React dependencies, no caching — isomorphic TS
 * that runs identically in the browser, Node.js sync scripts, and Deno
 * Edge Functions.
 *
 * Imported by:
 *   - contentVectorMapping.ts (client, wraps with in-memory cache)
 *   - scripts/sync-content.ts (Node.js batch sync)
 *   - supabase/functions/sync-incremental/index.ts (Deno Edge Function)
 */

// Inlined from tasteVector.ts — keeps this file self-contained for Supabase Edge Function
// deployment (CLI only uploads direct imports, not transitive ones without Docker).
// Source of truth: src/lib/taste/tasteVector.ts — keep in sync if those types change.
...
const META_DIMENSIONS = ['tone', 'pacing', 'era', 'popularity', 'intensity'] as const;

export const ALL_DIMENSIONS = [...GENRE_DIMENSIONS, ...META_DIMENSIONS] as const;

type GenreDimension = typeof GENRE_DIMENSIONS[number];
type Dimension = typeof ALL_DIMENSIONS[number];
type TasteVector = Record<Dimension, number>;
...
function clampVector(v: TasteVector): TasteVector {
  const out = { ...v };
...
/**
 * Pure function: map TMDb content metadata to a 24-dimensional TasteVector.
 ...
 *
 * No caching — callers that need caching should wrap this (see contentVectorMapping.ts).
 */
