...
- Migration 094 (D16 (a)): `remove_member` (revokes the open invite too, so a removed person cannot rejoin from the same link) and `revoke_invite`; verify script `verify-094-household-remove-member.sql`. Written and dry-run in PGlite with four mutation checks; NOT applied (Joe). The app's two PostgREST write shapes (item `ON CONFLICT DO NOTHING`, reaction upsert) were also run against 093's grants and RLS in PGlite.
