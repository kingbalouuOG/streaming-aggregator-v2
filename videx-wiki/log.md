...
- **Not done, deliberately:** applying 083 (MCP apply was permission-gated — production DDL stays Joe's), the deploy (manual; and it must follow the table), the cleanup. Run list and the four decisions with SQL: docs/plans/2026-09-11-002-plan-sync-tmdb-id-cleanup.md. The sync was left running as Joe asked; until the deploy it keeps writing vendor ids, which the walk rebuild removes.
...
- Probe branch `debug/touch-probe` is diagnostic only and is never merged.
