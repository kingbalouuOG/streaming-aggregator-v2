...
- Pending link v2 (`intent`, `invite`; v1 rows discarded; pure half in `src/lib/growth/pendingLinkRecord.ts`); resumes never clear a list link; clearing keyed on focus (IN-GR-040 narrowed); name gate and single `/auth` (IN-GR-044 closed).
