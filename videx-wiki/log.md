...
## [2026-09-18] query | IN-GR-048 Android OTA runtime mismatch root-caused and fixed in CI
- Cause measured in CI (diagnostic runs 35367402382, 35368260937): Gradle's `:app:createReleaseUpdatesResources` fingerprints mid-build, after Gradle has rewritten `node_modules/@react-native-masked-view/masked-view/android/src/main/AndroidManifest.xml` and while `expo-updates-gradle-plugin` holds transient output. Clean + `.env` = after prebuild = `eas fingerprint:generate` = `1c1a04ff…`; in-task `431c25b0…`; post-build `5672bf8c…`. The prebuilt-`android/` / config-plugin hypothesis was wrong. `app.config.js` also makes `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID` part of the fingerprint.
- Fix (branch fix/android-ota-fingerprint): `android-release.yml` resolves the runtime before prebuild and passes `EXPO_UPDATES_FINGERPRINT_OVERRIDE` to Gradle (as EAS Build does), then asserts the AAB carries it; `ota-update.yml` checks reachability before publishing. Proof on 04decff: branch AAB (run 35369263918) `1c1a04ff…` = eas-cli (run 35369264476) `1c1a04ff…`; iOS stays `2f0341fd…`.
- versionCode 17 can never receive an OTA; the first reachable Android OTA follows the next versionCode build.
- Updated: wiki/concepts/operations/ota-updates.md, wiki/registers/parking-lot.md (IN-GR-048 + counts), index.md.

## [2026-09-18] ingest | G2 H2: household app (list screen, picker, reactions, invite share, migration 094)
- App: `native/src/app/list/[id].tsx` (members' list; `?invite=` join with `household_joined` on a first join and the pending link cleared only on success; signed-out and non-member preview from `GET /v1/list/:id/preview`, stubbed until H3), Watchlist tab "Mine" / household picker with the shared list inline, "Add to {household}" on the detail page, Profile → Household (create, members, invite, revoke, remove, leave), `ShareTarget` list arm (`via=household`; owners mint an invite). Shared lib `src/lib/household/` (errors and copy for every code, typed RPC wrappers, item and reaction writes, list preview contract, households assembly) with vitest suites; `withShareAttribution` takes a `via` (default `share`), `buildListShareCopy` added.
- Migration 094 (D16 (a)): `remove_member` (revokes the open invite too, so a removed person cannot rejoin from the same link) and `revoke_invite`; verify script `verify-094-household-remove-member.sql`. Dry-run in PGlite with four mutation checks, then applied by Joe 2026-09-18 with the verify script run; typegen-check green. The app's two PostgREST write shapes (item `ON CONFLICT DO NOTHING`, reaction upsert) were also run against 093's grants and RLS in PGlite.
- Updated: wiki/entities/codebase/hooks.md, wiki/entities/codebase/components.md (new Household section), wiki/entities/codebase/event-taxonomy.md (`household_joined` emitted from the list screen; list shares), wiki/entities/codebase/rpcs.md (094), wiki/entities/codebase/migrations.md (094 row).

## [2026-09-18] ingest | G2 H3: shared-list links (feat/growth-h3-list-links)
- Worker: `GET /list/:id` is a real page (uuid check, `loadListPreview`, 60 s cache `LIST_PAGE_TTL_SECONDS`, noindex, `recordPreview` with a list object) and `GET /v1/list/:id/preview` returns `{id, name, household_name, count, posters, members}` (60 s, 404 JSON). No JWT member route (RLS reads, plan §11b). `?invite=` re-applied after the cache read into the deep link and the Play referrer; `PAGE_CACHE_VERSION` v3 → v4. og:title uses the household name (every v1 list is named "Shared").
- Inbound: `parseInboundLink` routes `/list/{uuid}` (https and `videx://`) to `/list/{id}` with `invite` (uuid, list links only); `inboundHref` composes `/list/{id}?invite={token}`; Play referrer keys `l=` and `i=` (a present `l` wins over `t` and `r`).
- Pending link v2 (`intent`, `invite`; v1 rows discarded; pure half in `src/lib/growth/pendingLinkRecord.ts`); resumes never clear a list link; clearing keyed on focus (IN-GR-040 narrowed); name gate before a resume on both sign-in paths (`auth.tsx` and the provider path in `OnboardingFlow.tsx`) and single `/auth` (IN-GR-044 closed).
- Created: wiki/entities/codebase/videx-api-worker.md. Updated: wiki/concepts/decisions/adr-015-object-urls-and-inbound-links.md, wiki/concepts/techniques/inbound-deep-linking.md, wiki/entities/codebase/event-taxonomy.md, wiki/registers/parking-lot.md, index.md.

## [2026-09-18] query | G2 H2 and H3 reconciled; H4 handoff written
- Plan §11c (H2: list screen, tab picker, Add to household, Profile → Household, list share arm; D16 (a) delivered with removal also revoking the open invite) and §11d (H3: /list/:id page and preview JSON at 60s, token never cached, PAGE_CACHE_VERSION v4, pending link v2 with intent, IN-GR-044 closed, IN-GR-040 narrowed). Release shape: G2 ships as 2.6.0 (iOS build 15 / versionCode 18) because no Play binary ≤17 can take an OTA (IN-GR-048).
- H4 handoff `docs/plans/2026-09-18-005-handoff-growth-h4-nudges.md`: pushPolicy pure module (cap groups, global floor 3/24h, quiet hours Europe/London), migration 095 (type CHECKs, nullable title columns, two partial unique indexes, list_id/nudge_window/push_id, 15-minute cron in the 059 pattern), `_shared/expoPush.ts` extraction, send-nudges function (adds since last nudge, bundled, claim-then-send, <30s), app routing through parseInboundLink and a Household activity toggle, IN-GR-041 (JWT + delivery ownership) and IN-GR-026 (push_id) fold-ins, dashboard §7, policy wording for Joe (D15).

## [2026-09-18] ingest | G2 H4 household nudges (migration 095, send-nudges, IN-GR-041, IN-GR-026)
- Built on `feat/growth-h4-nudges` (one PR; Joe merges after the D15 policy wording is approved and 095 is applied and verified live).
- `_shared/pushPolicy.ts` (pure, vitest): cap groups `titles` (arrival, leaving_soon) and `household` (household_nudge), one push per group per 20h, global floor 3 per rolling 24h, quiet hours 22:00 to 08:00 Europe/London via `Intl`, `nudgeWindowStart` = current UTC hour. Tests either side of both boundaries and across both 2026 clock changes.
- Migration 095 written, dry-run against production (rolled back): type CHECKs, nullable title columns, `list_id` / `nudge_window` / `push_id`, shape CHECK plus an insert trigger for `list_id`, nudge unique key, `send-nudges-15m` cron. Unique indexes kept non-partial (PostgREST cannot infer a partial index: 42P10, verified live); `verify-095-nudges.sql`.
- `_shared/expoPush.ts` extracted from send-notifications unchanged; send-notifications now caps by group plus floor and writes a `push_id` per push. `send-nudges` function + pure `compose.ts` (selection, bundle copy, payload `videx://list/{id}`).
- App: `src/lib/notifications/pushTap.ts` (route through `parseInboundLink`, `household_nudge` origin type, `notification_opened` metadata `push_id` and `kind`); Profile → Notifications gains Household activity.
- Worker: `notification_opened` needs a JWT (401) and delivery ownership (403). Dashboard §6c/§6d take the third type; §7 household loop. Privacy policy and in-app mirror: household activity wording (Joe approves).
- Updated: wiki/concepts/architecture/notifications-v1.md, wiki/entities/codebase/event-taxonomy.md, database-schema.md, migrations.md, videx-api-worker.md, wiki/registers/parking-lot.md (closed IN-GR-026, IN-GR-041; filed IN-GR-049 tonight nudge, IN-GR-050 list reply moment).
- Later 2026-09-18: Joe applied 095. Verify checks run against production (forced rollback): all 7 passed, no residue; cron `send-nudges-15m` jobid 28. `database.types.ts` regenerated (095 additions only). Still Joe's: deploy send-notifications and send-nudges, D15 wording, OTA, device check.
