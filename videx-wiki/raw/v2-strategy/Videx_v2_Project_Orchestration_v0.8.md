...
**Status:** v0.8 — E&P Hardening track kickoff (ENG-1, 2026-06-10). §11 records the Engine & Platform Hardening track lock — the six confirmed decisions (D1–D6) from the E&P brief §9 plus the ADR-011/ADR-012 supersession path at PLAT-3. §3.4 adds the planned ENG-1 migrations (044 `user_interest_centroids`, 045 training-extract view) and corrects the 040 row to what actually shipped (`040_editor_notes.sql`, Phase 6 — in repo, not yet applied to production). Brief at `docs/v2/Videx_v2_Engine_and_Platform_Hardening_Brief_v0.2.md` is authoritative for E&P phase scope. No process changes from v0.7.

> **Cross-phase knowledge base.** Active strategy / orchestration / parking-lot / phase-summary documents live under `docs/v2/` in this repo (source of truth). An **LLM-maintained Karpathy-style wiki** sits at `videx-wiki/` in this repo (Git-tracked, committed alongside phase work) — it synthesises the `docs/v2/` material plus runbooks / post-mortems / source-doc snapshots into a queryable knowledge graph (entities, concepts, registers). **Read `videx-wiki/AGENTS.md` first** before operating in that vault — it defines the schema and the three workflows (ingest / query / lint). Last ingest: Phase 5.5 close-out (2026-05-15).
**Version:** 0.8

**Changes from v0.7 (E&P Hardening kickoff, 2026-06-10):**
- §11 (Confirmed Decisions): added the **Engine & Platform Hardening track lock** — phase sequence ENG-1 → REPO-1 → PLAT-1 → PLAT-2 → PLAT-3 → (Phase 6 launch, parallel track) → ENG-2, all before v3/Phase 7, with the six locked decisions D1–D6 confirmed by Joe 2026-06-10 (ENG-first ordering, Cloudflare Workers + Hono, K=3 centroid cap, client-fallback retirement one release post-PLAT-3, React.lazy without a router, plain `workers/api/` folder).
- §11: recorded the **ADR-011 / ADR-012 supersession path** — the `_shared/` mirror and the Edge-render-with-client-fallback arrangement remain binding through ENG-1/REPO-1/PLAT-1/PLAT-2; a new ADR at PLAT-3 cutover supersedes both (single server-side engine, mirror + `shared-tree-drift` + parity apparatus deleted).
- §3.4: added planned ENG-1 rows — **044** `user_interest_centroids` (K ≤ 3 interest centroids, owner-scoped RLS, GDPR RPC extensions) and **045** training-extract view (impressions → same-session outcomes, the ENG-2 dataset shape). The E&P brief §12 anticipated one ENG-1 migration; the view is split out so the table and the analytics surface stay independently revertable.
- §3.4: **corrected the 040 row.** The number was consumed by `040_editor_notes.sql` (Phase 6 PR-AD, merged with the v3 editorial redesign PR #7) — verified 2026-06-10 that `public.editor_notes` does **not** exist in production, so the migration is in-repo-only. The IN-458 typed-pairs work that previously held the 040 slot remains deferred and takes the next free number when it lands.

**Changes from v0.6 (Phase Search V2 close-out, 2026-05-13):**
- §3.4: migration 041 ✅ Applied — `user_feature_flags` table (composite-PK `(user_id, flag_name, enabled)`, RLS scoped to owning user). First per-user feature flag store; backs the `search_semantic` opt-in.
- §11 (Confirmed Decisions): added the Phase Search V2 search architecture lock — Mode A (TMDb + Postgres ILIKE parallel + per-item availability via `useItemAvailability`), Mode C semantic via `embed-query` Edge function + 60/25/15 shared ranker behind the per-user flag.
- §11: added the per-user feature flag pattern lock (composite-PK table, typed `getFlag` accessor with module-scope cache, RLS to owner, Studio toggling acceptable for the prototype-users rollout pattern).
- New §3.4 Phase Search V2 actuals note covering the single-migration outcome, the `embed-query` deploy + `search-semantic-eval` workflow scaffolding, the 20-query fixture deferral, and the in-phase revert of A8's pre-call RPC to parallel + per-item.

**Changes from v0.5 (Phase 5 kickoff portion, 2026-04-30):**
...
- §3.4: the placeholder "034+ taste_profiles RLS" row removed. Replaced with explicit Phase 5 rows for migrations 036–040.
- New Phase 4.5 redirect / IN-466 / cold-start actuals note in §3.4.

**Changes from v0.6 kickoff (Phase 5 close-out, 2026-05-06):**
- §3.4: migration 036 ✅ Applied — `taste_profiles` RLS enabled.
- §3.4: migration 037 ✅ Applied — `marked_watched` dropped from `user_interactions.event_type` CHECK; pre-flight count was 0.
- §3.4: migration 038 ✅ Applied — `username_available` SECURITY DEFINER RPC; replaces wide-open anon policy on `profiles`.
- §3.4: migration 039 ✅ Applied — cron jobs Vault-backed JWT (storage migration without cryptographic rotation; rotation deferred to Phase 6 pending Supabase JWT-format secret keys).
- §3.4: migration 040 deferred to Phase 5.5 / 6 — IN-458 typed pairs; 0.8% collision rate, low impact.
...
| 036 | `036_taste_profiles_rls.sql` | Phase 5 (pre-launch blocker) | ✅ Applied (2026-05-06) | Enable RLS on `taste_profiles` with `FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id)`. Pre-existing gap surfaced by Phase 4 security review. GDPR / privacy blocker for public launch. Service-role bypass preserves Edge Function reads. Verified: anon SELECT returns 0 rows. |
| 037 | `037_drop_marked_watched_from_check.sql` | Phase 5 | ✅ Applied (2026-05-06) | Drop `'marked_watched'` from `user_interactions.event_type` CHECK constraint. Pre-flight count was 0 → clean DROP / ADD swap. Read-side filters cleaned up in same PR. The `exit_reason` payload value (Spec v0.3.2 line 237) is unrelated and stays. Verified: CHECK rejects `'marked_watched'` inserts. |
| 038 | `038_profiles_username_lookup_rpc.sql` | Phase 5 (IN-XPS-002) | ✅ Applied (2026-05-06) | Replace `FOR SELECT USING (true)` anon policy on `profiles` with SECURITY DEFINER RPC. Onboarding signup flow goes through `supabase.rpc('username_available', { check_username })`. Verified: anon SELECT denied, RPC returns boolean. |
| 039 | `039_cron_jobs_vault_jwt.sql` | Phase 5 (IN-XPS-004) | ✅ Applied (2026-05-06) | Move the inline service-role JWT out of `006_cron_schedule.sql` and three `supabase/cron/*.sql` files into Supabase Vault. Re-create four cron jobs (`daily-content-sync`, `embed-new-titles`, `enrich-new-titles`, `refresh-service-fingerprints`) using `vault.decrypted_secrets`. **NOT a cryptographic rotation** — same JWT value, just different storage location. Full rotation deferred to Phase 6 pending Supabase tooling (see Phase 5 summary §5.4). Verified: `count(*) FROM cron.job WHERE command LIKE '%Bearer ey%'` = 0; runtime confirmation pending first scheduled run. |
| 040 | `040_editor_notes.sql` | Phase 6 (PR-AD) | ⚠ In repo, NOT applied (verified 2026-06-10) | `editor_notes` table backing the Home/For You EditorsNote primitive — kicker/teaser/body, publish window (`published_at`/`expires_at`), partial index on the publish window, anon + authenticated SELECT RLS, service_role ALL, one seed row. Landed via the Phase 6 editorial redesign PR (#7) but never pushed to production — `public.editor_notes` absent as of the E&P kickoff check. Apply before Phase 6 editorial features go live. **Numbering note:** this consumed the slot previously pencilled for the IN-458 typed-pairs migration (`get_available_tmdb_ids` → `(tmdb_id, media_type)` pairs, 0.8% collision fix); that work remains deferred and takes the next free number when it lands. |
| 041 | `041_user_feature_flags.sql` | Phase Search V2 (B1) | ✅ Applied (2026-05-13) | Composite-PK `user_feature_flags (user_id, flag_name, enabled, created_at, updated_at)` table. RLS scoped to owning user via `auth.uid()` policies on SELECT + `FOR ALL`. `updated_at` trigger. Backs the `search_semantic` per-user flag (Cluster B opt-in). First per-user feature flag store — pattern is reusable for any future Joe-first / prototype-users rollout. |
| 042 | `042_delete_own_account.sql` | Phase 5.5 (IN-XPS-006) | ✅ Applied (2026-05-15) | Captures `public.delete_own_account()` SECURITY DEFINER RPC in source control with belt-and-braces defensive explicit DELETEs across 8 user-scoped tables (`card_impressions`, `user_interactions`, `taste_profiles`, `user_services`, `user_genres`, `watchlist`, `onboarding_events`, `user_feature_flags`) before the `profiles` + `auth.users` deletes. Pre-cut H5 audit confirmed every table has `ON DELETE CASCADE` chains so the live behaviour was correct; this migration ships the same semantics + survives a future cascade-rule regression. C11 smoke test on a throwaway user (113 `card_impressions` + 6 onboarding events): every count = 0 post-delete; `auth.users` row gone. Verified: `prosecdef = true`, `search_path = public,pg_temp`, EXECUTE granted to authenticated. |
| 043 | `043_export_user_data.sql` | Phase 5.5 (IN-PX-35) | ✅ Applied (2026-05-15) | GDPR Article 20 (data portability) + Article 15 (access). `public.export_user_data()` SECURITY DEFINER RPC returns a `jsonb` object keyed by user-scoped table, scoped to `auth.uid()` on every query. `card_impressions` capped to last 90 days (matches migration 014's daily-aggregate rollup). Empty tables return `'[]'::jsonb` via COALESCE. Frontend wires `@capacitor/filesystem` on native + Blob download on web. Verified: smoke-test from Studio raises on `auth.uid()` = NULL (the auth gate working); production export from the app returns full user payload. |
| 044 | `044_user_interest_centroids.sql` | Phase ENG-1 | ✅ Applied (2026-06-10, by Joe via Studio after the CC permission gate) | New `user_interest_centroids` table — `(user_id, slot smallint, centroid vector(1536), weight real, updated_at)`, PK `(user_id, slot)`, `slot` constrained 0–2 (K ≤ 3 per E&P brief §9 D3), owner-scoped RLS (`auth.uid() = user_id`) + service_role ALL, FK to `auth.users` ON DELETE CASCADE. Also CREATE OR REPLACEs `delete_own_account()` (042) and `export_user_data()` (043) to include the new user-scoped table. `taste_profiles.taste_vector_v2` continues to be maintained as the single-centroid summary (mood-room affinity + fallback). |
| 045 | `045_training_extract_view.sql` | Phase ENG-1 | ✅ Applied (2026-06-10, by Joe via Studio) | Training-extract view (`security_invoker = true`) joining `card_impressions` to same-session outcome events (`thumbs_up`, `watchlist_add`, `deep_link_click`, `watched`) — the ENG-2 learned-re-ranker dataset shape, defined at ENG-1 per brief §3.5 so position/exploration fields accumulate correctly from day one. No new tables. |
| 046 | `046_drop_title_genres_credits.sql` | Phase REPO-1 | ⏳ Written, awaiting Joe's apply (destructive — manual snapshot first per §8.3) | Drop `title_genres` + `title_credits` — the parked drop from the deferred-items register (E&P brief §4.3-2). Both confirmed 0 rows in production 2026-06-10; created in 001 for a v1-era cache superseded by Phase 0.5's denormalised `titles` columns (017, IN-102/IN-106). No live code references; `database.types.ts` blocks removed in the same commit (typegen-check verifies post-apply). |
...
**Phase 5.5 actuals note:** Phase 5.5 shipped **two migrations (042, 043) + 28 source-tree commits** across three clusters. Cluster A (quality / type / performance — 9 commits): C1 regenerated `database.types.ts` post-migrations 036–041 + removed 28 `as any` casts at the Supabase boundary (plan listed 4; actual sweep was 7× larger but mechanical) + added `typegen-check.yml` CI; C2 moved `ViewingContext` to `types.ts` and narrowed at the `profiles.viewing_context` boundary in both client + Edge; C3 collapsed `buildRowFromPool` to an options object; C4 added MMR partial-coverage fallback to `applyGenreSpread` with `bailedOut` signal and named bail-threshold constants; C5 shipped the embedding cache (24h client localStorage + per-Edge-instance Map) + Float32Array map shape + cached cosine norms (~3× MMR hot-loop speedup) — cache key simplified post-review to `userId + taste_profiles.updated_at` (no derived hashes); C6a/C6b added vitest rig + 10 pure-function tests; C7 extended `edge-fn-jwt-guard` to scan central `supabase/config.toml`; C8 deleted `supabase/cron/*.sql` mirror files (migration 039 sole source of truth); C9 shipped the `foryou-parity` golden probe with `--update-golden` flag + JWT refresh script + activation on 2026-05-15. Cluster B (legal disclosures — 7 commits): migrations 042 + 043 (above) plus delete-account UI flip with type-username confirm + Privacy Policy / ToS authored under Joe's review and rendered via `react-markdown` from `?raw` Vite imports + "Download my data" Blob/Filesystem wiring. Both legal docs carry a mandatory lawyer-vetting caveat at the footer; pre-launch solicitor review filed as new parking-lot IN-XPS-014 (hard pre-launch blocker). Cluster C (catalogue-gap closure — 2 commits + diagnostics): C17 investigation diagnosed the gap at 5,446 distinct missing tmdb_ids (84% Prime, 83% movies, 74% pre-2010, 79% popularity-q1) caused by TMDb `/discover` 500-page popularity-cap excluding Prime back-catalogue. **Critical pipeline-split finding:** the `daily-content-sync` cron writes only to `streaming_availability`, never to `titles` — `titles` rows come only from the manual `scripts/sync-content.ts:stageTmdb` script. So `scripts/backfill_missing_titles.ts` (whose query `streaming_availability LEFT JOIN titles WHERE titles IS NULL` catches everything missing) IS the recurring fix. C18 executed 2026-05-15: 5,446 missing → 2,698 upserted into `titles`, 2,748 TMDb-404 (deleted stubs stay missing forever), 0 errored. Verified post-run: missing_n = 2,748 (= 404 count); titles grew from 20,140 → 22,838. Two new parking-lot follow-ups filed: **IN-XPS-014** (solicitor review, pre-launch blocker) and **IN-PX-50** (wrap backfill script in scheduled Edge Function, Phase 6 automation). Two C18 bug-fixes shipped mid-run: PostgREST default-page-size of 1000 was under-counting missing rows (paginated SA + titles pulls in 1000-row chunks); TMDb empty-string `release_date` was rejected as Postgres date syntax (null-coerce on empty + length-zero). Phase 5.5 summary at `docs/v2/phase-summaries/phase-5.5-summary.md` is authoritative for what shipped vs deferred.

**Phase Search V2 actuals note:** Phase Search V2 shipped the single planned migration (041) — composite-PK `user_feature_flags` table with RLS scoped to the owning user. Cluster A (filtered search, A1–A11) landed unflagged: typed `FilterState` with URL serialisation, FilterSheet rewrite (9 sections — DECADE dropped post-A2 review, UK RATING dropped per locked decision), new primitives (`ServiceTile`, `ActiveFilterPill`, `MoodChip`), `<ContentCard>` 3-state vocabulary (on-service / on-service-rent-or-buy / off-service tint), as-you-type suggestions, recent searches, instrumentation (`emitSearch` augmentation + `card_impressions.metadata.{mode, query_hash, filter_set_hash}`), regression tests. Cluster B (semantic search, B1–B6) landed flag-gated: `embed-query` Edge function (JWT-protected, in-memory LRU 1h TTL/1000-entry cap), `semanticRetrieval` shared between `src/lib/recommendations-v2/search/` and `supabase/functions/_shared/recommendations-v2/search/` with 60/25/15 (relevance/taste-fit/recency) weights, mode dispatch in `useSearch`, `SearchModeIndicator` + `SearchSemanticCTA`, eval rig (`scripts/test/search-semantic-eval.ts`) + CI workflow (`.github/workflows/search-semantic-eval.yml`). A8 (pre-call `getAvailableTmdbIds` RPC) was reverted mid-phase to parallel TMDb + Postgres + per-item `useItemAvailability` flow after live testing surfaced p50 + round-trip-flash regressions. Eval workflow lands with a 2-query fixture stub — 20-query authorship is a Joe deliverable that gates flag-flip to non-Joe users (not the B6 commit, not Joe's own usage). Three-agent close-out review: `security-sentinel` SAFE-TO-MERGE, `kieran-typescript-reviewer` SHIP-IT with two trivial fix-after-merge items (IN-PX-39, plus an additional dependent for IN-PX-21), `performance-oracle` FIX-FIRST → fixed in commit `9ec4868` (mood-chip Mode A clobber). See Phase Search V2 summary for full details.
...
**Operational automation vs schema evolution.** The migration sequence is reserved strictly for schema evolution — `CREATE TABLE`, `ALTER TABLE`, indexes, constraints, extension installs. Operational automation that schedules recurring jobs lives in the migration that owns the consuming schema or workstream — see migration 039 (`cron_jobs_vault_jwt`) which registers all four current pg_cron schedules (`embed-new-titles`, `enrich-new-titles`, `refresh-service-fingerprints`, `daily-content-sync`) in one place. The two pg_cron jobs in migration 014 (`card_impressions_rollup`, `pg_partman_maintenance`) live similarly inside that migration because they are tightly coupled to the lifecycle of the `card_impressions` table.

> **Phase 5.5 (IN-PX-31).** The mirror `supabase/cron/*.sql` files were deleted. The runtime never read them — they were duplicate copies of the registrations in migration 039 with no enforcement contract. Going forward `supabase/cron/` is intentionally empty (see the README there); cron registrations belong inside the migration that owns the consuming schema or workstream.
...
**Pipeline code mirroring per ADR-011:** locked (April 2026). `supabase/functions/_shared/recommendations-v2/` and `_shared/taste-v2/` mirror `src/lib/recommendations-v2/` and `src/lib/taste-v2/` for Edge Function consumption. The `shared-tree-drift` GitHub Actions workflow fails any PR that touches one tree without the other. Escape hatch: `drift-allowed: <reason>` marker in PR body or commit message for emergency one-sided patches. Long-term consolidation into a runtime-portable shared package filed as IN-467 — revisit after 1-2 months of operational experience. Phase Search V2 extended the mirror to `_shared/recommendations-v2/search/semanticRetrieval.ts` (the client adapter imports from `_shared/` directly rather than maintaining a separate copy — single source for the ranker, drift-by-construction impossible).

**Search architecture per Phase Search V2:** locked (2026-05-13). Mode A (Browse/Search tab keyword search) fans out to three parallel sources via `Promise.all` — TMDb `/search/movie`, TMDb `/search/tv`, Postgres `searchTitlesByText` ILIKE substring against the ~20K UK-available titles cache. The Postgres source closes the compound-word gap TMDb's tokeniser misses ("salt" never matches "Saltburn"). Availability + tint state flows through a shared `useItemAvailability` hook reused by filter-only browse. Mode B (entity search) is out of scope for this phase. Mode C (semantic search) is per-user flag-gated via `search_semantic` in `user_feature_flags` — opt-in CTA appears when Mode A returns ≤2 results on a free-text query; tap dispatches via `embed-query` Edge function (OpenAI `text-embedding-3-small`, in-memory LRU cache) → `match_titles_by_vector` RPC → 60/25/15 (relevance/taste-fit/recency) shared ranker → post-filter from `FilterState`. Eval rig at `scripts/test/search-semantic-eval.ts` with precision@10 + MRR thresholds; flag-flip from Joe-only to other prototype users gated on the eval going green against the 20-query fixture (authorship pending — IN-PX-40).

**Per-user feature flag pattern per Phase Search V2:** locked (2026-05-13). Composite-PK `(user_id, flag_name)` `user_feature_flags` table (migration 041), RLS scoped to owning user. Typed accessor `src/lib/featureFlags.ts:getFlag(name, fallback)` with module-scope Promise cache. Flag read once on consuming surface mount — no live subscription. Manual Studio toggle takes effect on next remount (tab switch or hard reload). Supports the Joe-first / prototype-users rollout pattern. Flag-flip UI surface deferred (IN-PX-41).

**Engine & Platform Hardening track (E&P):** locked (2026-06-10 — all six §9 decisions confirmed by Joe; brief v0.2 at `docs/v2/Videx_v2_Engine_and_Platform_Hardening_Brief_v0.2.md` is authoritative for phase scope). Phase sequence: **ENG-1** (multi-interest retrieval & signal quality) → **REPO-1** (docs/repo hygiene) → **PLAT-1** (client data layer & rendering) → **PLAT-2** (API edge & content proxy) → **PLAT-3** (single engine, server-side feed + cache) → [Phase 6 launch, parallel track] → **ENG-2** (learned re-ranker, data-gated on ≥5–10K impressions with ≥500 positive outcomes). Runs entirely before v3 / Phase 7. Branch naming continues the house convention: `phase-eng-1-multi-interest`, `phase-repo-1-hygiene`, `phase-plat-1-client-data-layer`, …. The six locked decisions:

- **D1 — ENG-1 before PLAT-3.** Engine work pays the ADR-011 `_shared/` mirror tax one final time; the mirror update is mechanical and `shared-tree-drift` CI protects it.
- **D2 — Cloudflare Workers + Hono** for the PLAT-2 proxy and the PLAT-3 feed: zero cold starts (the documented Achilles heel of the Supabase Edge path), Cache API + CDN built in, and wrangler bundles from anywhere in the repo — dissolving the ADR-011 constraint rather than working around it. Only new recurring cost in the track: Workers Paid ~£4/mo from PLAT-3.
- **D3 — Interest-centroid K fixed at 3** for ENG-1 (`user_interest_centroids.slot` 0–2); adaptive K is an ENG-2-era refinement.
- **D4 — Client pipeline post-PLAT-3:** kept as offline/error fallback for one release after cutover, then deleted. ADR required at that point.
- **D5 — No router adoption in PLAT-1:** `React.lazy` + Suspense per top-level page only; the existing tab-state + Capacitor back-button stack stays. Router is YAGNI until web distribution matters.
- **D6 — Worker code lives in a plain `workers/api/` folder** (created empty with a README stub in REPO-1 so PLAT-2 lands into an agreed location); workspace split only if a second app target appears.

**ADR-011 / ADR-012 supersession path per E&P track:** locked (2026-06-10). The `_shared/recommendations-v2/` + `_shared/taste-v2/` mirror commitment (ADR-011) and the Edge-render-primary-with-client-fallback arrangement (ADR-012) remain binding through ENG-1, REPO-1, PLAT-1 and PLAT-2 — ENG-1 changes to either tree MUST be mirrored, enforced by `shared-tree-drift`. At PLAT-3 cutover a new ADR supersedes both: one engine implementation imported directly by the Worker (`GET /v1/foryou`), feed cache keyed `userId : taste_vector_updated_at : sliderHash`, parity probe run once as final validation then retired, the `_shared/` mirrors + `shared-tree-drift` workflow + `warmup-foryou` Variant A hack + the 1.5s-timeout fallback dance all deleted. Client pipeline survives one release as fallback, then is deleted per D4. LOC/CI deleted is tracked in the PLAT-3 phase summary as a first-class deliverable.
...
*End of Project Orchestration v0.8. E&P Hardening track locks (D1–D6 + ADR-011/012 supersession path) recorded at ENG-1 kickoff, 2026-06-10. Sections 1–10 and 12 retain their original Phase-0-era text as historical record; §3.4 and §11 are the live sections.*
