...
| 23 | CORS allow-list on user-callable Edge Functions | Phase 5 IN-XPS-013 | Eng | ✅ Closed Phase 5 — `_shared/cors.ts` echoes Origin only when allow-listed (`https://localhost`, `^http://localhost(:port)?$`, `VIDEX_ALLOWED_DEV_ORIGINS` env hook). Applied to `render-foryou-rows` and `label-anchor-room`; today its importers are `label-anchor-room` and `embed-query`. `capacitor://localhost` dropped 2026-09-14 (IN-DEP-002 follow-up), here and in the `videx-api` Worker. |
...
| 13 | Supabase Pro backup off-site copy (monthly `pg_dump`) | Risks register R-015; supabase-backup-restore runbook | Eng + Joe (secrets) | ⚠ 2026-07-06 (H0 Stream D) — workflow `.github/workflows/db-backup.yml` shipped (monthly encrypted `pg_dump` → GitHub artifact). **Blocked on Joe adding 2 repo secrets** (`SUPABASE_DB_URL`, `BACKUP_GPG_PASSPHRASE`) + one manual run — console-actions §2. |
