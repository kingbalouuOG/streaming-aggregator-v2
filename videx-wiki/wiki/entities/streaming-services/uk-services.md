...
- ⚠ **Adding a service is a client/server contract, and the Worker must move first.** The id allow-list exists twice: `ServiceId` in the app and `VALID_SERVICE_IDS` in `workers/api/src/index.ts`, which guards both `/v1/foryou` and `/v1/home`. The Worker deploys on merge to `main`; an app build can reach a device before that. Seen 2026-09-10: a device build carrying the wave-1 ids met a Worker that still held the old ten, and every feed call returned `{"error":"unknown service id"}` (400) **before auth**. A user with a mixed stack sees thin results; a user holding only new services sees "Couldn't load your feed" everywhere. Deploy the Worker first, or ship both together.
