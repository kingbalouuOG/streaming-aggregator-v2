...
| 040 | `040_available_tmdb_ids_with_media_type.sql` | Phase 5 (IN-458) | **deferred to Phase 6** | Extend `get_available_tmdb_ids` to return `(tmdb_id, media_type)` pairs. Closes 0.8% movie/TV id collision rate. Real but small impact; deferred from Phase 5.5 to keep scope tight. **Migration shape**: must be additive (new function `get_available_tmdb_id_pairs`), not in-place — never swap RPC return shape when clients consume it. (NB: a different `040_editor_notes.sql` lives in the repo from the v3 editorial-redesign branch but is unapplied to remote — see `useHomeContent.ts:79` for the retained `as any` cast it requires.) |
