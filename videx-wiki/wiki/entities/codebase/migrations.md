...
| 048–082 | *(not yet catalogued on this page — see `supabase/migrations/` and the sync-pipeline runbook for 062–078)* | | | |
| 083 | `083_sa_show_map.sql` | IN-SY-001 (2026-09-11) | **in repo, awaiting Joe** (production DDL is an explicit-Joe action; the MCP apply was permission-gated) | `sa_show_map (sa_show_id text PK, tmdb_id, media_type, title, source, first_seen_at, last_seen_at)` + index on `(tmdb_id, media_type)`; RLS on, no policies (service-role only, same as 063). The vendor-show-id → TMDb-id map that `sync-incremental` resolves `/changes` rows against — the feed carries no TMDb id. Must be applied **before** the fixed `sync-incremental` is deployed: the function treats a missing table as a fetch failure and marks the run failed rather than writing. Reversible with `DROP TABLE`. See [post-mortem](../../concepts/operations/solutions/sync-vendor-show-id-in-tmdb-id.md). |
