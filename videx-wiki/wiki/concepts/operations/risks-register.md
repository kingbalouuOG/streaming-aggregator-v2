...
| R-029 | A denormalising trigger silently no-ops when the row it updates does not exist yet | High | Found live 2026-08-27, one day after B3 shipped: `count_available_services_drift()` returned **821**, and all 821 had the same shape — `stored='{}'` with real availability, all titles created inside that morning's 05:00 backfill window, and for **821 of 821** the `streaming_availability` rows were written BEFORE the `titles` row existed. 075's trigger lives only on `streaming_availability` and recomputes with `UPDATE titles ... WHERE tmdb_id = ...`; with no title row that affects **zero rows and raises nothing**, and the title is then inserted with the column at its `'{}'` default, never revisited. B3 is what made it user-visible: Home's spotlights filter on `available_services` in SQL, so an empty column makes a title invisible to them however healthy its availability rows are — the *newest* titles, which an unfrozen catalogue exists to surface, silently never appear, compounding every backfill run. Fixed by migration 077: an `AFTER INSERT` trigger on `titles` so ordering stops mattering, plus a repair and a drift-is-zero assertion in the same transaction. **Generalises: a trigger that maintains a denormalised column on another table is only correct if it fires from BOTH sides — an UPDATE that matches nothing is indistinguishable from success.** Note R-027 predicted drift, but from bulk loads bypassing the trigger; no amount of trigger-disabling discipline would have caught write ordering. The daily drift check is what caught it. | Eng |
