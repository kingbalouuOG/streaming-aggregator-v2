...
| 5.5 — Quality + legal hardening | not started | — | (planned) | Quality cluster (IN-PX-21–28, 31–33, plus Search V2 follow-ups IN-PX-39): regen `database.types.ts`, embedding fetch cache, MMR partial-coverage fallback, Float32 + norm precompute, tests, options-object refactors, mirror tree consolidation, `catch (err: any)` cleanup in `useSearch.ts`. Legal cluster (IN-XPS-006, IN-PX-34, IN-PX-35): delete-account UI flip + RPC source-of-truth migration, functional Privacy Policy + ToS, GDPR Article 20 data export. See parking lot. | — |
| v3 editorial redesign + pre-launch chrome | 2026-05-09 (closed) | `v3design-system-exploration` (PR #7, squash-merge `2dfc685`) | 040 (`editor_notes`) | Full editorial-magazine redesign across 35+ matrix rows in `docs/v3-design/redesign-plan.md`. New tokens (paper / ink / cream surfaces, atmosphere / service tints, glass-scrim weights, badge halo, star, type ramp at `--t-body 13`). New components: `MagazineHero`, `EditorsNote`, `LongRead`, `NumberedChart`, `FreeTonight`, `WideCard` (real TMDb backdrops), `CalendarList`, `TopAppBar`, `genreIcons`. ContentCard / WatchlistPage / FilterSheet / ProfilePage / DetailPage / OnboardingFlow / auth screens all migrated to the editorial vocabulary. Bottom-nav glyphs normalised to a 4→20 (16×16) optical-weight bbox. ServiceBadge fallback uses `var(--svc-*)`; ServiceStack outline removed (Disney+ / Prime brand colours bleed edge-to-edge). Independent code review (kieran-typescript-reviewer) ran pre-merge; all must-fixes addressed. Post-merge polish on Android device: hero/LongRead full-bleed on mobile, TopAppBar non-sticky with `calc(env(safe-area-inset-top) + 20px)` symmetric padding (the earlier `pt-5 safe-top` mix was silently overriding the env-inset), CategoryFilter retained on Home as static chips with the trailing filter button suppressed. Parking-lot follow-ups filed: IN-V3-001 (Long Read data layer), IN-V3-002 (taste-v2 surface for hero match% + mood), IN-V3-003 (mood refiner real wiring — UI shipped behind `MOOD_REFINER_ENABLED=false` flag). | [redesign-plan.md](../../../../docs/v3-design/redesign-plan.md) |
