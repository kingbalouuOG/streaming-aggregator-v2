...
status: G0/G1 shipped (v2.5.0 build 14 / versionCode 17 submitted 18 Sept 2026); G2 H1 LIVE (migration 093, PR #211, 18 Sept); H2 app and H3 links handoffs written 18 Sept, running in parallel; D16 (member removal) open; H4 nudges after both; then G3 taste cards, G4 SEO, G5 household recs
...
- **S5 verification (2026-09-16..17, PRs #195 2.5.0, #196, #197, #201):** device matrix passed on both platforms with the fixes above; the first iOS build after the new entitlements needed a local interactive EAS credentials pass (stale provisioning profiles); Confirm email flip moved to the public 2.5.0 release (2.4.0 is live on the App Store); evidence snapshot `docs/v2/phase-summaries/evidence/growth-s5-growth-events.md` (C5 account deletion removed the iPhone install's rows, IN-GR-009).
