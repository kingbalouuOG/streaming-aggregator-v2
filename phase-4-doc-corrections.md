...
9. **`taste_profiles` table has no RLS** (security review finding M1): The table does not have `ENABLE ROW LEVEL SECURITY` in any migration. Any authenticated or anon user with the Supabase anon key can read/write any user's taste profile. Pre-existing issue, not introduced by Phase 4. Needs a migration to add RLS with `auth.uid() = user_id` policies. Pre-launch blocker (GDPR/privacy).
